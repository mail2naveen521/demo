...
def run_drift_check():
    ...
    sys.modules["_drift_importing"] = 1  # tell extract_baseline not to run _main()
    import extract_baseline as extractor
    del sys.modules["_drift_importing"]  # clean up
...
# SCRIPT ENTRY POINT
#
# detect_drift.py is run directly via wsadmin -f detect_drift.py
# When imported by manage_drift.py, that script sets:
#   detector._IS_IMPORT = 1
# before calling any functions, so run_drift_check() is skipped.
# =============================================================================

# Only run when executed directly via wsadmin -f detect_drift.py
# Importers (manage_drift.py) set _drift_importing in sys.modules first
