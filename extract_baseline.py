...
CELL_NAME    = ""  # resolved at runtime in build_snapshot()
...
def get_servers():
    ...
    for sid in _cfg_list("Server"):
        ...
        # JVM settings (heap sizes, JVM args, generic args etc.)
        jvm_cfg = ""
        for jid in _cfg_list("JavaVirtualMachine", sid):
            jvm_cfg = _show(jid)

        # Thread pools (WebContainer, ORB, etc.)
        tps = {}
        for tid in _cfg_list("ThreadPool", sid):
            tps[_attr(tid, "name")] = _show(tid)

        # Log file config: SystemOut.log / SystemErr.log size and rollover
        # Stored under JavaProcessDef -> StreamRedirect in WAS config
        # WAS stores stdout first, stderr second under JavaProcessDef
        log_cfg = {}
        for jpd_id in _cfg_list("JavaProcessDef", sid):
            sr_list = _cfg_list("StreamRedirect", jpd_id)
            stream_names = ["stdout", "stderr"]
            idx = 0
            for sr_id in sr_list:
                if idx < len(stream_names):
                    log_cfg[stream_names[idx]] = _show(sr_id)
                else:
                    log_cfg["stream_%d" % idx] = _show(sr_id)
                idx += 1

        # RAS logging service (trace specification, log detail level)
        ras_cfg = ""
        for ras_id in _cfg_list("RASLoggingService", sid):
            ras_cfg = _show(ras_id)
...
# SCRIPT ENTRY POINT
#
# wsadmin -f extract_baseline.py baseline
#   sets sys.argv = ["baseline"]  (args only, NOT the script path)
#
# When imported by detect_drift.py or manage_drift.py those scripts set:
#   extract_baseline._IS_IMPORT = 1
# before calling any functions, so _main() is skipped.
# =============================================================================
...
# Only run _main() when wsadmin calls this script directly.
# wsadmin -f extract_baseline.py baseline  -> sys.argv[0] = "baseline"
# wsadmin -f extract_baseline.py snapshot  -> sys.argv[0] = "snapshot"
# When imported by detect_drift or manage_drift, sys.argv[0] will be
# something else (e.g. "update-baseline", empty, or the other script name).
# This check is more reliable than the sys.modules sentinel in Jython 2.1.
if len(sys.argv) > 0 and (sys.argv[0] == "baseline" or sys.argv[0] == "snapshot"):
    _main()
