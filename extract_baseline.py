...
def build_snapshot():
    ...
    # AdminControl.getHost() is a direct wsadmin method – always reliable.
    try:
        dmgr_host = AdminControl.getHost()
    except:
        dmgr_host = "unknown"

    # Read WAS version from the product properties file on disk –
    # avoids all MBean attribute name differences across fix-pack levels.
    was_version = "unknown"
    try:
        props_path = AdminControl.getConfigRepository() + "/properties/version/WAS.product"
