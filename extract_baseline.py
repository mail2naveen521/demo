...
def build_snapshot():
    ...
    # Resolve DMGR host – use AdminControl.getNode() / getHost() which are
    # always available, with MBean attribute lookup as a fallback.
    try:
        dmgr_host = AdminControl.getHost()
    except:
        dmgr_host = "unknown"

    # Resolve WAS version – getAttribute name differs across fix-packs;
    # try safe alternatives before falling back to "unknown".
    try:
        dmgr_mbean = AdminControl.queryNames("type=DeploymentManager,*")
        was_version = AdminControl.getAttribute(dmgr_mbean, "version")
    except:
        try:
